  ...
  for (const prefix of ['Eladó', 'Vevő']) {
    ...
    const option = (
      ...
      return {
        ...
        friendlyName,
        priority: idx++,
        replacementString: withPrefix(
          replacementString,
          prefix === 'Eladó' ? 'seller' : 'buyer',
        ),
        minimumValue: min,
        maximumValue: max,
    ...
    await contractOptionRepository.insert([
      ...
      option(
        OptionType.STRING,
        `${prefix} édesanyjának neve`,
        'mother_name',
        2,
      ),
      option(
        OptionType.DATE,
        `${prefix} édesanyjának születési dátuma`,
        'mother_birthdate',
      ),
      option(OptionType.STRING, `${prefix} nemzetisége`, 'nationality', 2),
      option(
        OptionType.PERSONAL_IDENTIFIER,
        `${prefix} személyazonossági okiratának típusa`,
        'id_number_type',
      ),
      option(
        OptionType.STRING,
        `${prefix} személyazonossági okiratának száma`,
        'id_number',
      ),
