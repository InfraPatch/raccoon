...
export const downloadFilledItemAttachment = async (email: string, attachmentId: number): Promise<IDownloadAttachmentResponse> => {
  const { attachment } = await getFilledItemAttachment(email, attachmentId);

  return await downloadAttachment('item', attachment.filename, attachment.filledItemId);
};
