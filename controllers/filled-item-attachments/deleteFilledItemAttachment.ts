...
import {
  getFilledItemAttachment,
  FilledItemAttachmentResponse,
} from './getFilledItemAttachment';
...
export const deleteFilledItemAttachment = async (
  email: string,
  attachmentId: number,
): Promise<void> => {
  const { attachment, user }: FilledItemAttachmentResponse =
    await getFilledItemAttachment(email, attachmentId);
