...
const EditContractForm = ({ contractProp }: EditContractFormProps) => {
  ...
  const handleFormSubmit = async (
    { friendlyName, description }: EditContractFormRequest,
    { setSubmitting }: FormikHelpers<EditContractFormRequest>,
  ) => {
    try {
      const response: UpdateContractAPIResponse =
        await apiService.contracts.updateContract({
          id: contract.id,
          friendlyName,
          description,
          file,
