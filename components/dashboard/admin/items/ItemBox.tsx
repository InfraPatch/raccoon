...
const ItemBox = ({ item } : DashboardItemBoxProps) => {
  ...
  return (
    <Box title={item.friendlyName}>
      <span className="py-3">{t('dashboard:admin.items.last-updated')}: {item.updatedAt}</span>
      <h1 className="py-3">{item.description || t('dashboard:admin.items.default-description')}</h1>
