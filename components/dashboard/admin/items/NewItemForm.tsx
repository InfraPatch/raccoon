...
import {
  NewItemAPIRequest,
  NewItemAPIResponse,
} from '@/services/apis/items/ItemAPIService';
...
const NewItemForm = () => {
  ...
  const handleFormSubmit = async (
    { friendlyName, description }: NewItemAPIRequest,
    { setSubmitting }: FormikHelpers<NewItemAPIRequest>,
  ) => {
    try {
      const response: NewItemAPIResponse = await apiService.items.newItem({
        friendlyName,
        slug,
        description,
      });
