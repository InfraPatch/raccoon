...
const FilledContractFieldsForm = ({
  filledContract,
  onChange,
  partyType,
}: FilledContractFieldsFormProps) => {
    ...
    const newFields = filledContract.contract.options
      ...
      .map((option) => {
        return {
          ...
          type: option.type,
          value:
            filledContract.options.find((a) => a.option.id === option.id)
              ?.value || '',
  ...
  const handleFormSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await apiService.filledContracts.fillFilledContract(
        filledContract.id,
        { options: fields },
      );
      ...
      if (err.response?.data?.error) {
        toaster.danger(
          t(
            `errors:contracts.${err.response.data.error}`,
    ...
    <form onSubmit={handleFormSubmit}>
      ...
      <div className="form-field">
        <Button size={ButtonSize.MEDIUM} type="submit" disabled={saving}>
          {t('dashboard:contracts.data.fill-submit')}
        </Button>
      </div>
    </form>
  );
