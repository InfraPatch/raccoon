...
const FilledContractSignatures = ({
  contract,
}: FilledContractSignaturesProps) => {
  const { t } = useTranslation('dashboard');

  const signatures: IFilledContractSignature[] = [];
      ...
      <div className={dataContainerClassNames}>
        <Box title={t('dashboard:contracts.data.signatures')}>
          <div className="text-sm flex flex-wrap overflow-hidden justify-center text-center items-center -mx-3">
            {signatures.map(
              (signature: IFilledContractSignature, idx: number) => {
                const onLoad = () => {
                ...
                const type =
                  signature.type === PartyType.BUYER
                    ? 'buyer'
                    : signature.type === PartyType.SELLER
                    ? 'seller'
                    : 'witness';
                  ...
                  <div key={signature.id} className={className}>
                    <article className="rounded-lg shadow-xl bg-field">
                      <img
                        ...
                        src={`/api/filled-contracts/${contract.id}/signatures/${signature.id}`}
                        onLoad={onLoad}
                      />
                      ...
                      <p className="pb-2">
                        {t(`dashboard:contracts.list.${type}-field`)}
                      </p>
                    </article>
                  </div>
                );
